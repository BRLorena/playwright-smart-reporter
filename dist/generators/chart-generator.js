...
function generateTrendChart(data) {
    ...
    if (summaries.length < 1) {
      ...
      <div class="trend-section">
        <div class="trend-header">
          <div class="trend-title">${(0, icon_provider_1.icon)('bar-chart-2')} Test Run Trends</div>
          <div class="trend-subtitle">Collecting data...</div>
        </div>
        <div class="trend-message">
          <p>${(0, icon_provider_1.icon)('trending-up')} <strong>Trends will appear after 2+ test runs.</strong></p>
    ...
    const generateBarChart = (chartData, getValue, maxValue, color, yAxisLabel, formatValue, avgColor = '#38bdf8', gradientId = 'trendGrad') => {
      ...
      `;
        ...
        const curvePoints = chartData.map((d, i) => {
        ...
        if (n >= 2) {
            const tangents = curvePoints.map((_, i) => {
                if (i === 0)
                    return [curvePoints[1][0] - curvePoints[0][0], curvePoints[1][1] - curvePoints[0][1]];
                if (i === n - 1)
                    return [curvePoints[n - 1][0] - curvePoints[n - 2][0], curvePoints[n - 1][1] - curvePoints[n - 2][1]];
      ...
      `;
        ...
        if (an >= 2) {
            const at = avgPointPairs.map((_, i) => {
                if (i === 0)
                    return [avgPointPairs[1][0] - avgPointPairs[0][0], avgPointPairs[1][1] - avgPointPairs[0][1]];
                if (i === an - 1)
                    return [avgPointPairs[an - 1][0] - avgPointPairs[an - 2][0], avgPointPairs[an - 1][1] - avgPointPairs[an - 2][1]];
            ...
            const clampAvgY = (y) => Math.max(avgYMin, Math.min(avgYMax, y));
    ...
    const passRateChart = generateBarChart(allSummaries, (s) => s.passRate || 0, 100, '#22c55e', 'Pass Rate (%)', (val) => `${val}%`, '#38bdf8', 'trendGrad-passrate');
    // Generate duration bar chart
    const durationChart = generateBarChart(allSummaries, (s) => Math.round((s.duration || 0) / 1000), Math.ceil(maxDuration / 1000), '#a855f7', 'Duration (s)', (val) => `${val}s`, '#38bdf8', 'trendGrad-duration');
    // Generate flaky tests bar chart
    const flakyChart = generateBarChart(allSummaries, (s) => s.flaky || 0, maxFlaky, '#eab308', 'Flaky Tests', undefined, '#38bdf8', 'trendGrad-flaky');
    // Generate slow tests bar chart
    const slowChart = generateBarChart(allSummaries, (s) => s.slow || 0, maxSlow, '#f97316', 'Slow Tests', undefined, '#38bdf8', 'trendGrad-slow');
    return `
    <div id="trends-section" class="trend-section collapsible-section">
      <div class="trend-header" onclick="toggleSection('trends-section')">
        <div style="display: flex; align-items: center;">
          <div class="trend-title">${(0, icon_provider_1.icon)('bar-chart-2')} Test Run Trends</div>
          <span class="section-toggle">${(0, icon_provider_1.icon)('chevron-down', 14)}</span>
        </div>
        <div class="trend-subtitle">Last ${allSummaries.length} runs</div>
      </div>
      <div class="trend-controls" onclick="event.stopPropagation()">
        <button class="chart-toggle-btn" onclick="toggleAllAvg(this)" title="Toggle 3-run average on all charts">${(0, icon_provider_1.icon)('trending-up', 12)} <span>3-run avg</span></button>
        <button class="chart-toggle-btn" onclick="toggleAllMode(this)" title="Switch all charts to bar view">${(0, icon_provider_1.icon)('bar-chart-2', 12)} <span>Bars</span></button>
      </div>

      <!-- All Charts in Grid -->
      <div class="section-content all-charts-grid">
        <div class="line-chart-container">
          <h4 class="chart-title">${(0, icon_provider_1.icon)('check-circle')} Pass Rate</h4>
          ${passRateChart}
        </div>
        <div class="line-chart-container">
          <h4 class="chart-title">${(0, icon_provider_1.icon)('clock')} Duration</h4>
          ${durationChart}
        </div>
        <div class="line-chart-container">
          <h4 class="chart-title">${(0, icon_provider_1.icon)('shuffle')} Flaky Tests</h4>
          ${flakyChart}
        </div>
        <div class="line-chart-container">
          <h4 class="chart-title">${(0, icon_provider_1.icon)('hourglass')} Slow Tests</h4>
          ${slowChart}
        </div>
      </div>
    </div>
