...
class StabilityScorer {
    constructor(stabilityThreshold = 70, thresholds) {
    ...
    scoreTest(test) {
        ...
        const overall = Math.round(flakiness * this.weightFlakiness +
     ...
     */
    getGrade(score) {
        if (score >= this.gradeA)
            return 'A';
        if (score >= this.gradeB)
            return 'B';
        if (score >= this.gradeC)
            return 'C';
        if (score >= this.gradeD)
            return 'D';
        return 'F';
    }
    /**
