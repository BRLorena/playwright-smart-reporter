...
function getAttachmentIcon(contentType) {
    if (contentType.startsWith('image/'))
        return (0, icon_provider_1.icon)('image');
    if (contentType.startsWith('video/'))
        return (0, icon_provider_1.icon)('film');
    if (contentType.startsWith('audio/'))
        return (0, icon_provider_1.icon)('volume-2');
    if (contentType.startsWith('text/'))
        return (0, icon_provider_1.icon)('file');
    if (contentType.includes('json'))
        return (0, icon_provider_1.icon)('braces');
    if (contentType.includes('pdf'))
        return (0, icon_provider_1.icon)('file-text');
    if (contentType.includes('zip') || contentType.includes('tar') || contentType.includes('gzip'))
        return (0, icon_provider_1.icon)('package');
    return (0, icon_provider_1.icon)('paperclip');
}
/**
...
function getBrowserIcon(browser) {
    ...
    if (name.includes('chromium') || name.includes('chrome'))
        return (0, icon_provider_1.icon)('globe', 12);
    if (name.includes('firefox'))
        return (0, icon_provider_1.icon)('flame', 12);
    if (name.includes('webkit') || name.includes('safari'))
        return (0, icon_provider_1.icon)('compass', 12);
    if (name.includes('edge'))
        return (0, icon_provider_1.icon)('hexagon', 12);
    return (0, icon_provider_1.icon)('monitor', 12);
}
/**
...
function getAnnotationIcon(type) {
    ...
    if (t === 'slow')
        return (0, icon_provider_1.icon)('hourglass', 12);
    if (t === 'fixme' || t === 'fix')
        return (0, icon_provider_1.icon)('wrench', 12);
    if (t === 'skip')
        return (0, icon_provider_1.icon)('skip-forward', 12);
    if (t === 'fail' || t === 'expected-failure')
        return (0, icon_provider_1.icon)('x-circle', 12);
    if (t === 'issue' || t === 'bug')
        return (0, icon_provider_1.icon)('bug', 12);
    if (t === 'flaky')
        return (0, icon_provider_1.icon)('shuffle', 12);
    if (t === 'todo')
        return (0, icon_provider_1.icon)('pencil', 12);
    return (0, icon_provider_1.icon)('pin', 12);
}
/**
 * Generate a single test card
 */
function generateTestCard(test, showTraceSection, quarantinedTestIds) {
    ...
    <div id="card-${cardId}" class="test-card"
      ...
      <div class="test-card-header" ${hasDetails ? `onclick="toggleDetails('${cardId}', event)"` : ''}>
        ...
        <div class="test-card-right">
          ...
          ${hasDetails ? `<span class="expand-icon">${(0, icon_provider_1.icon)('chevron-right', 14)}</span>` : ''}
        </div>
      </div>
...
function generateTestDetails(test, cardId, showTraceSection) {
    ...
    if (test.history && test.history.length > 0) {
        ...
        historyDetails += `
	      <div class="detail-section">
	        <div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('bar-chart-2')}</span> Run History (Last ${test.history.length} runs)</div>
    ...
    if (test.steps.length > 0) {
        ...
        bodyDetails += `
      <div class="detail-section">
        <div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('clock')}</span> Step Timeline</div>
    ...
    if (showTraceViewer) {
        bodyDetails += `
      <div class="detail-section">
        <div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('bar-chart-2')}</span> Trace</div>
              ...
              <div class="trace-row">
                <div class="trace-meta">
                  <div class="trace-file">
                    <span class="trace-file-icon">${(0, icon_provider_1.icon)('package')}</span>
                    <span class="trace-file-name" title="${safeTrace}">${fileName}${suffix}</span>
                  </div>
                  <div class="trace-path" title="${safeTrace}">${safeTrace}</div>
                </div>
                <div class="trace-actions">
                  <a href="${safeTrace}" class="attachment-link" download>${(0, icon_provider_1.icon)('download')} Download</a>
                  <a href="#" class="attachment-link" data-trace="${safeTrace}" onclick="return viewTraceFromEl(this)">${(0, icon_provider_1.icon)('search')} View</a>
                </div>
              </div>
            `;
        }).join('')}
    ...
    if (test.videoPath) {
      ...
      <div class="detail-section">
        ...
        <div class="attachments">
          <a href="file://${test.videoPath}" class="attachment-link" target="_blank">${(0, icon_provider_1.icon)('film')} Video</a>
        </div>
      </div>
    `;
    }
    ...
    if (test.attachments?.custom && test.attachments.custom.length > 0) {
        ...
        bodyDetails += `
      <div class="detail-section">
        <div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('paperclip')}</span> Custom Attachments</div>
        <div class="attachments">
 ...
 */
function generateGroupedTests(results, showTraceSection, attention = { newFailures: new Set(), regressions: new Set(), fixed: new Set() }, quarantinedTestIds) {
    // Group tests by file
    ...
    <div id="group-${groupId}" class="file-group">
      <div class="file-group-header" onclick="toggleGroup('${groupId}')">
        ...
        <span class="file-group-name">${(0, icon_provider_1.icon)('file')} ${(0, utils_1.escapeHtml)(file)}</span>
...
function generateNetworkLogsSection(networkLogs, cardId) {
    ...
    const entriesHtml = entries.map((entry, idx) => {
      ...
      <div class="network-entry ${statusClass}" data-entry-id="${entryId}">
        <div class="network-entry-header" onclick="toggleNetworkEntry('${entryId}')">
          ...
          <span class="network-expand-icon">${(0, icon_provider_1.icon)('chevron-right', 14)}</span>
        </div>
    ...
    <div class="detail-section network-logs-section">
      <div class="detail-label">
        <span class="icon">${(0, icon_provider_1.icon)('globe')}</span> Network Logs
