...
exports.generateExecutivePdf = generateExecutivePdf;
...
function computeAverageGrade(results) {
...
// ---------------------------------------------------------------------------
let pageCount = 0;
function ensureSpace(doc, y, needed, version, scheme, footerText) {
...
function drawPageBg(doc, scheme) {
    doc.save();
    doc.rect(0, 0, PAGE_W, PAGE_H).fill(scheme.pageBg);
    doc.restore();
}
function drawPageHeader(doc, scheme) {
    doc.save();
...
function drawFooter(doc, version, page, scheme, brandingFooter) {
    ...
    const left = brandingFooter || `Generated by playwright-smart-reporter v${version}`;
    doc.text(left, MARGIN, y + 10, { width: CONTENT_W * 0.6 });
    doc.text(`Page ${page}  \u00B7  ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}`, MARGIN + CONTENT_W * 0.6, y + 10, { width: CONTENT_W * 0.4, align: 'right' });
    doc.restore();
}
// ---------------------------------------------------------------------------
// Drawing primitives
...
function drawHorizontalBar(doc, x, y, w, h, segments, scheme) {
    const total = segments.reduce((s, seg) => s + seg.value, 0);
    if (total === 0) {
        doc.save();
...
// ---------------------------------------------------------------------------
function drawTitleBlock(doc, data, version, scheme) {
    let y = MARGIN;
    ...
    const ciParts = [datePart];
    if (data.ciInfo) {
        if (data.ciInfo.branch)
            ciParts.push(`Branch: ${data.ciInfo.branch}`);
        if (data.ciInfo.commit)
            ciParts.push(`Commit: ${data.ciInfo.commit.substring(0, 7)}`);
        if (data.ciInfo.buildId)
            ciParts.push(`Build: ${data.ciInfo.buildId}`);
    }
    doc.save();
...
// ---------------------------------------------------------------------------
function drawVerdictBanner(doc, data, scheme) {
    let y = 108;
...
// Section: KPI cards
// ---------------------------------------------------------------------------
function drawKpiCards(doc, data, scheme) {
    const y = 178;
    const stats = computeStats(data.results);
    const duration = Date.now() - data.startTime;
...
// ---------------------------------------------------------------------------
function drawDistributionSection(doc, data, startY, scheme) {
    let y = startY;
    const stats = computeStats(data.results);
    const sectionH = 160;
    // Section title
    ...
    for (const r of data.results) {
        const key = r.file;
        if (!suiteMap.has(key))
            suiteMap.set(key, { total: 0, passed: 0, failed: 0, flaky: 0 });
        const s = suiteMap.get(key);
        s.total++;
        if (r.outcome === 'flaky')
            s.flaky++;
        else if (r.outcome === 'unexpected' && (r.status === 'failed' || r.status === 'timedOut'))
            s.failed++;
        else if (r.status === 'passed' || r.outcome === 'expected')
            s.passed++;
    }
    if (suiteMap.size > 1) {
        const tableY = ly + 56;
...
// ---------------------------------------------------------------------------
function drawQualityGates(doc, result, startY, version, scheme, footerText) {
    ...
    doc.text(passed ? '\u2713  All gates passed' : '\u2717  Gates failed', overallX + 8, y + 5, { width: overallW - 16 });
    doc.restore();
    return y + 32;
}
...
// ---------------------------------------------------------------------------
function drawTrendCharts(doc, summaries, startY, version, scheme, footerText) {
...
// ---------------------------------------------------------------------------
function drawFailuresSection(doc, data, startY, version, scheme, footerText) {
...
// ---------------------------------------------------------------------------
function drawFailureClusters(doc, clusters, startY, version, scheme, footerText) {
...
// ---------------------------------------------------------------------------
function drawQuarantineSection(doc, entries, threshold, startY, version, scheme, footerText) {
...
// ---------------------------------------------------------------------------
function drawSuiteBreakdown(doc, data, startY, version, scheme, footerText) {
    const suiteMap = new Map();
    for (const r of data.results) {
        if (!suiteMap.has(r.file))
            suiteMap.set(r.file, { total: 0, passed: 0, failed: 0, flaky: 0, skipped: 0, duration: 0 });
    ...
    doc.font('Helvetica-Bold').fontSize(7).fillColor(scheme.bodyText);
    let hx = MARGIN + 8;
    for (const [label, w] of [['File', colWidths[0]], ['Total', colWidths[1]], ['Passed', colWidths[2]], ['Failed', colWidths[3]], ['Flaky', colWidths[4]], ['Duration', colWidths[5]], ['Pass Rate', colWidths[6]]]) {
...
// Main export
// ---------------------------------------------------------------------------
function generateExecutivePdf(data, outputDir, basename, theme) {
    ...
    const doc = new pdfkit_1.default({
        ...
        info: {
            ...
            Author: 'playwright-smart-reporter',
        },
        autoFirstPage: true,
        bufferPages: true,
    });
