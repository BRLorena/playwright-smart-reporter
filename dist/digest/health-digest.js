...
class HealthDigest {
    ...
    calculatePassRateTrend(summaries) {
    ...
    findNewFlakyTests(tests, periodStart) {
        const result = [];
        for (const [testId, entries] of Object.entries(tests)) {
            const inPeriod = entries.filter(e => !e.skipped && new Date(e.timestamp).getTime() >= periodStart);
            const beforePeriod = entries.filter(e => !e.skipped && new Date(e.timestamp).getTime() < periodStart);
            if (inPeriod.length === 0)
                continue;
            const currentScore = inPeriod.filter(e => !e.passed).length / inPeriod.length;
            if (currentScore < FLAKY_THRESHOLD)
                continue;
            ...
            if (beforePeriod.length > 0) {
                ...
                if (previousScore >= FLAKY_THRESHOLD)
                    continue; // Already flaky, not new
            }
            result.push({
                testId,
    ...
    findRecoveredTests(tests, periodStart) {
        const result = [];
        for (const [testId, entries] of Object.entries(tests)) {
            const beforePeriod = entries.filter(e => !e.skipped && new Date(e.timestamp).getTime() < periodStart);
            if (beforePeriod.length === 0)
    ...
    findPerformanceTrends(tests, periodStart) {
        const result = [];
        for (const [testId, entries] of Object.entries(tests)) {
            const inPeriod = entries.filter(e => !e.skipped && new Date(e.timestamp).getTime() >= periodStart);
            const beforePeriod = entries.filter(e => !e.skipped && new Date(e.timestamp).getTime() < periodStart);
            if (inPeriod.length === 0 || beforePeriod.length === 0)
                continue;
            const avgBefore = beforePeriod.reduce((sum, e) => sum + e.duration, 0) / beforePeriod.length;
            const avgInPeriod = inPeriod.reduce((sum, e) => sum + e.duration, 0) / inPeriod.length;
            if (avgBefore === 0)
                continue;
...
exports.HealthDigest = HealthDigest;
