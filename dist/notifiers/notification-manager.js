...
const pagerduty_notifier_1 = require("./pagerduty-notifier");
const email_notifier_1 = require("./email-notifier");
const custom_webhook_notifier_1 = require("./custom-webhook-notifier");
function buildContext(results, startTime) {
...
function evaluateConditions(conditions, ctx, results, comparison) {
    if (results.length === 0)
        return false;
    if (conditions.minFailures !== undefined && ctx.failed < conditions.minFailures)
        return false;
    if (conditions.maxPassRate !== undefined && ctx.passRate > conditions.maxPassRate)
        return false;
    if (conditions.tags && conditions.tags.length > 0) {
        const hasTagFailure = results.some(r => r.outcome === 'unexpected' &&
...
class NotificationManager {
    ...
    async notify(results, startTime, comparison) {
        ...
        for (const config of this.configs) {
            try {
                ...
                switch (config.channel) {
                    case 'slack': {
                        const notifier = new slack_notifier_1.SlackNotifier(config.config.webhookUrl);
                        await notifier.sendMessage(message);
                        break;
                    }
                    case 'teams': {
                        const notifier = new teams_notifier_1.TeamsNotifier(config.config.webhookUrl);
                        await notifier.sendMessage(message);
                        break;
                    }
                    case 'pagerduty': {
                        const notifier = new pagerduty_notifier_1.PagerDutyNotifier(config.config.routingKey);
                        await notifier.trigger(message, ctx.failed, ctx.total);
                        break;
                    }
                    case 'email': {
                        const notifier = new email_notifier_1.EmailNotifier(config.config);
                        await notifier.send(message, ctx);
                        break;
                    }
                    case 'webhook': {
                        const notifier = new custom_webhook_notifier_1.CustomWebhookNotifier(config.config.url);
