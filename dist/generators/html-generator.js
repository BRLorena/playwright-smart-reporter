...
function generateFileTree(results) {
    ...
    return Array.from(fileGroups.entries()).map(([file, stats]) => {
        ...
        return `
      <div class="file-tree-item ${statusClass}" data-file="${(0, utils_1.escapeHtml)(file)}" onclick="filterByFile('${(0, utils_1.escapeJsString)(file)}')">
        <span class="file-tree-icon">${(0, icon_provider_1.icon)('file')}</span>
 ...
 */
function generateTestListItems(results, showTraceSection, attention = { newFailures: new Set(), regressions: new Set(), fixed: new Set() }, quarantinedTestIds) {
 ...
 */
function generateSparkline(values, color, gradientId, fixedMin, fixedMax) {
    if (values.length < 2)
        return '';
    const W = 200;
    const H = 60;
    ...
    const tangents = points.map((_, i) => {
        ...
        if (i === n - 1)
            return [points[n - 1][0] - points[n - 2][0], points[n - 1][1] - points[n - 2][1]];
    ...
    const clampSpkY = (y) => Math.max(spkYMin, Math.min(spkYMax, y));
 ...
 */
function generateOverviewContent(results, comparison, failureClusters, passed, failed, skipped, flaky, slow, newTests, total, passRate, totalDuration, history, qualityGateResult, quarantineEntries, quarantineThreshold, licenseTier, aiSuiteHealthSummary) {
    ...
    <div class="overview-section">
      <div class="section-header">
        <span class="section-icon">${(0, icon_provider_1.icon)('search')}</span>
        ...
        return `
          <div class="cluster-card" onclick="filterTests('failed'); switchView('tests');">
            <div class="cluster-header">
              <div class="cluster-icon">${(0, icon_provider_1.icon)('alert-triangle')}</div>
  ...
  ` : '';
    ...
    <div class="overview-section attention-section">
      <div class="section-header">
        <span class="section-icon">${(0, icon_provider_1.icon)('zap')}</span>
      ...
      <div class="quality-gate-card ${qualityGateResult.passed ? 'gate-passed' : 'gate-failed'}">
        ...
        <div class="gate-header">
          <div class="gate-title-row">
            <span class="section-icon">${(0, icon_provider_1.icon)('gauge')}</span>
        ...
        const ruleIcon = rule.skipped ? (0, icon_provider_1.icon)('circle', 14) : rule.passed ? (0, icon_provider_1.icon)('check', 14) : (0, icon_provider_1.icon)('x', 14);
        ...
        return `
            <div class="gate-rule-row">
              ...
              <span class="gate-rule-name">${(0, utils_1.escapeHtml)(label)}</span>
              <span class="gate-rule-values">${rule.skipped ? '(skipped)' : `${(0, utils_1.escapeHtml)(rule.actual)} ${(0, utils_1.escapeHtml)(rule.threshold)}`}</span>
            </div>`;
    }).join('')}
        </div>
      </div>
      ...
      <div class="quarantine-card">
        ...
        <div class="quarantine-header">
          <div class="quarantine-title-row">
            <span class="section-icon">${(0, icon_provider_1.icon)('lock')}</span>
        ...
        <div class="quarantine-entries">
          ${quarantineEntries.slice(0, 3).map(e => `
            <div class="quarantine-entry">
              <span class="quarantine-entry-title">${(0, utils_1.escapeHtml)(e.title)}</span>
      ...
      <div class="ai-health-card">
        <div class="ai-health-header">
          <div class="ai-health-title-row">
            <span class="section-icon">${(0, icon_provider_1.icon)('bot')}</span>
            <span class="ai-health-title">AI Health Summary</span>
          </div>
          <span class="premium-badge" style="font-size:9px;background:var(--accent-purple);color:#fff;padding:1px 5px;border-radius:3px;">Starter</span>
        </div>
        <div class="ai-health-body ai-markdown">${(0, utils_1.renderMarkdownLite)(aiSuiteHealthSummary)}</div>
      </div>
    </div>
  ` : (!hasPro ? `
    <div class="overview-section ai-health-section">
    ...
    <div class="upgrade-banner" id="upgradeBanner">
      <div class="upgrade-banner-content">
        <div class="upgrade-banner-text">
          <span class="upgrade-banner-icon">${(0, icon_provider_1.icon)('zap', 18)}</span>
      ...
      <div class="feature-usage-card">
        <div class="feature-usage-header">
          <div class="feature-usage-title-row">
            <span class="section-icon">${(0, icon_provider_1.icon)('package')}</span>
        ...
        <div class="feature-usage-columns">
          <div class="feature-usage-col">
            <div class="feature-usage-col-header active-header">${(0, icon_provider_1.icon)('check', 14)} ${activeFeatures.length} Active</div>
            ${activeFeatures.map(f => `<div class="feature-usage-item active">${(0, utils_1.escapeHtml)(f)}</div>`).join('')}
          </div>
          <div class="feature-usage-col">
            <div class="feature-usage-col-header locked-header">${(0, icon_provider_1.icon)('lock', 14)} ${lockedFeatures.length} Locked</div>
            ${lockedFeatures.map(f => `<div class="feature-usage-item locked" title="${(0, utils_1.escapeHtml)(f.desc)}">${(0, utils_1.escapeHtml)(f.name)}</div>`).join('')}
          </div>
        </div>
      </div>
    </div>
  ` : '';
    ...
    <div class="overview-section">
      <div class="section-header">
        <span class="section-icon">${(0, icon_provider_1.icon)('lightbulb')}</span>
      ...
      <div class="insights-grid">
        ${slowestTest ? `
          <div class="insight-card" onclick="selectTest('${(0, utils_1.sanitizeId)(slowestTest.testId)}'); switchView('tests');">
            <div class="insight-icon">${(0, icon_provider_1.icon)('hourglass', 24)}</div>
        ...
        ${mostFlakyTest && mostFlakyTest.flakinessScore && mostFlakyTest.flakinessScore > 0 ? `
          <div class="insight-card" onclick="selectTest('${(0, utils_1.sanitizeId)(mostFlakyTest.testId)}'); switchView('tests');">
            <div class="insight-icon">${(0, icon_provider_1.icon)('zap', 24)}</div>
            <div class="insight-content">
        ...
        <div class="insight-card clickable" onclick="switchView('trends')" title="View trends">
          <div class="insight-icon">${(0, icon_provider_1.icon)('trending-up', 24)}</div>
          <div class="insight-content">
...
function generateHtml(data) {
    ...
    const outputBasename = (0, utils_1.escapeHtml)(data.outputBasename ?? 'smart-report');
  ...
  <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600;700&family=Space+Grotesk:wght@400;500;600;700&display=swap" rel="stylesheet">`;
    ...
    const traceScript = enableTraceViewer ? (0, trace_viewer_generator_1.generateTraceViewerScript)() : '';
    ...
    const historyRunSnapshots = JSON.parse(document.getElementById('report-data-history-snapshots').textContent);`;
        ...
        externalJs = (enableTraceViewer ? (0, trace_viewer_generator_1.generateJSZipScript)() + '\n' : '')
            + cspDataInit + '\n' + jsAfterDeclarations
            + (traceScript ? '\n' + traceScript : '');
    }
...
<html lang="en"${options.theme?.preset && options.theme.preset !== 'default' ? ` data-theme="${(0, utils_1.escapeHtml)(options.theme.preset)}"` : ''}>
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${(0, utils_1.escapeHtml)(reportTitle)} — Smart Test Report</title>
  ...
  <meta property="og:title" content="${(0, utils_1.escapeHtml)(reportTitle)} — Smart Test Report">
    ...
    <header class="top-bar">
      <div class="top-bar-left">
        <button class="sidebar-toggle" onclick="toggleSidebar()" title="Toggle Sidebar (⌘B)" aria-label="Toggle sidebar navigation" aria-expanded="true" aria-controls="sidebar">
          <span class="hamburger-icon" aria-hidden="true">${(0, icon_provider_1.icon)('menu')}</span>
        </button>
        <div class="logo">
${branding?.logo ? `          <img class="logo-image" src="${(0, utils_1.escapeHtml)(branding.logo)}" alt="${(0, utils_1.escapeHtml)(reportTitle)} logo" height="28" />` : ''}
          <div class="logo-text">
            <span class="logo-title">${(0, utils_1.escapeHtml)(reportTitle)}</span>
${reportSubtitle ? `            <span class="logo-subtitle">${(0, utils_1.escapeHtml)(reportSubtitle)}</span>` : ''}
          </div>
        </div>
      ...
      <div class="top-bar-right">
        <button class="search-trigger" onclick="openSearch()" title="Search (⌘K)" aria-label="Search tests">
          <span class="search-icon-btn">${(0, icon_provider_1.icon)('search')}</span>
...
${options.exportPdf ? `            <button class="export-menu-item" onclick="showPdfPicker()" role="menuitem">
              <span>${(0, icon_provider_1.icon)('file-text')}</span> PDF Report
            </button>` : ''}
${options.exportJson ? `            <a class="export-menu-item" href="${outputBasename}-data.json" download role="menuitem" style="text-decoration:none;color:inherit;">
              <span>${(0, icon_provider_1.icon)('package')}</span> Full JSON Data
            </a>` : ''}
${options.exportJunit ? `            <a class="export-menu-item" href="${outputBasename}-junit.xml" download role="menuitem" style="text-decoration:none;color:inherit;">
              <span>${(0, icon_provider_1.icon)('tag')}</span> JUnit XML
            ...
            <button class="export-menu-item export-premium-placeholder" style="opacity:0.5;cursor:pointer;" onclick="showUpgradeModal('PDF Report', 'Generate polished PDF reports in three variants — Executive Summary, Detailed Failures, and Full Report — perfect for sharing with stakeholders.')">
              <span>${(0, icon_provider_1.icon)('file-text')}</span> PDF Report <span class="premium-badge" style="font-size:9px;background:var(--accent-purple);color:#fff;padding:1px 5px;border-radius:3px;margin-left:4px;">Starter</span>
            </button>
            <button class="export-menu-item export-premium-placeholder" style="opacity:0.5;cursor:pointer;" onclick="showUpgradeModal('Full JSON Data', 'Export your complete test results as structured JSON for custom dashboards, data pipelines, and integrations.')">
              <span>${(0, icon_provider_1.icon)('package')}</span> Full JSON Data <span class="premium-badge" style="font-size:9px;background:var(--accent-purple);color:#fff;padding:1px 5px;border-radius:3px;margin-left:4px;">Starter</span>
            </button>
            <button class="export-menu-item export-premium-placeholder" style="opacity:0.5;cursor:pointer;" onclick="showUpgradeModal('JUnit XML', 'Export JUnit XML for CI/CD integration with Jenkins, GitHub Actions, Azure DevOps, and more.')">
              <span>${(0, icon_provider_1.icon)('tag')}</span> JUnit XML <span class="premium-badge" style="font-size:9px;background:var(--accent-purple);color:#fff;padding:1px 5px;border-radius:3px;margin-left:4px;">Starter</span>
            </button>`}
          </div>
        </div>
        <div class="theme-dropdown" id="themeDropdown">
          <button class="theme-toggle" onclick="toggleThemeMenu()" title="Theme" aria-label="Change theme" aria-haspopup="true" aria-expanded="false">
            <span class="theme-toggle-icon" id="themeIcon">${(0, icon_provider_1.icon)('moon')}</span>
...
${!hasPro ? `            </div>` : ''}
    ...
    <div class="upgrade-modal-overlay" id="upgradeModal" role="dialog" aria-modal="true" aria-labelledby="upgradeModalTitle" style="display:none">
      <div class="upgrade-modal">
        ...
        <div class="upgrade-modal-icon">${(0, icon_provider_1.icon)('lock', 32)}</div>
      ...
      <nav class="sidebar-nav" aria-label="Main navigation">
        ...
        <div role="tablist" aria-labelledby="nav-section-label">
          <button class="nav-item active" data-view="overview" onclick="switchView('overview')" role="tab" aria-selected="true" aria-controls="view-overview">
            <span class="nav-icon" aria-hidden="true">${(0, icon_provider_1.icon)('bar-chart-2')}</span>
          ...
          <button class="nav-item" data-view="tests" onclick="switchView('tests')" role="tab" aria-selected="false" aria-controls="view-tests">
            <span class="nav-icon" aria-hidden="true">${(0, icon_provider_1.icon)('test-tube')}</span>
          ...
          <button class="nav-item" data-view="trends" onclick="switchView('trends')" role="tab" aria-selected="false" aria-controls="view-trends">
            <span class="nav-icon" aria-hidden="true">${(0, icon_provider_1.icon)('trending-up')}</span>
          ...
          <button class="nav-item" data-view="comparison" onclick="switchView('comparison')" role="tab" aria-selected="false" aria-controls="view-comparison">
            <span class="nav-icon" aria-hidden="true">${(0, icon_provider_1.icon)('scale')}</span>
          ...
          <button class="nav-item" data-view="gallery" onclick="switchView('gallery')" role="tab" aria-selected="false" aria-controls="view-gallery">
            <span class="nav-icon" aria-hidden="true">${(0, icon_provider_1.icon)('image')}</span>
            <span class="nav-label">Gallery</span>
          </button>
          ` : ''}
          ${showLive ? `
          <button class="nav-item" data-view="live" onclick="switchView('live')" role="tab" aria-selected="false" aria-controls="view-live">
            <span class="nav-icon" aria-hidden="true">${(0, icon_provider_1.icon)('radio')}</span>
...
${quarantineCount > 0 ? `            <button class="filter-chip attention-quarantine" data-filter="quarantined" data-group="health" onclick="toggleFilter(this)" aria-pressed="false">Quarantined (${quarantineCount})</button>` : ''}
      ...
      <div class="sidebar-footer">
        <div class="run-duration">
          <span class="duration-icon">${(0, icon_provider_1.icon)('clock')}</span>
          ...
          <div class="test-list-panel">
            ...
            <div class="test-list-content">
              ...
              <div class="empty-state" id="emptyState" style="display: none;">
                <div class="empty-state-icon">${(0, icon_provider_1.icon)('search', 24)}</div>
              ...
              <div class="test-tab-content" id="tab-by-file" role="tabpanel" aria-labelledby="tab-by-file-label">
                ${(0, card_generator_1.generateGroupedTests)(sortedResults, showTraceSection, attentionSets, quarantinedTestIds)}
              </div>
              <!-- By Status Tab -->
        ...
        return `
          ...
          <div class="test-detail-panel" id="test-detail-panel">
            <div class="detail-placeholder">
              <div class="placeholder-icon">${(0, icon_provider_1.icon)('test-tube', 32)}</div>
      ...
      <section class="view-panel${hasStarter ? '' : ' live-section-gated'}" id="view-live" role="tabpanel" aria-label="Live" style="display: none;"${!hasStarter ? ` onclick="showUpgradeModal('Live Run Controls', 'Run, cancel, and filter tests directly from the dashboard with real-time progress tracking.')"` : ''}>
        ...
        <div class="live-controls-wrapper" id="live-controls-wrapper">
          <div class="live-run-row live-run-row-hidden" id="live-run-row">
            ...
            <button class="live-filter-toggle" id="live-filter-toggle" onclick="toggleFilterPanel()" title="Filter tests">
              ${(0, icon_provider_1.icon)('filter', 16)} <span id="live-filter-toggle-label">Filters</span>
            </button>
          </div>
          <div class="live-filter-panel" id="live-filter-panel" style="display:none">
            <div class="live-filter-section">
              <div class="live-filter-section-header" onclick="toggleFilterSection('grep')">
                ...
                <span class="live-filter-chevron" id="live-filter-chevron-grep">${(0, icon_provider_1.icon)('chevron-down', 14)}</span>
              </div>
            ...
            <div class="live-filter-section">
              <div class="live-filter-section-header" onclick="toggleFilterSection('tags')">
                ...
                <span class="live-filter-chevron" id="live-filter-chevron-tags">${(0, icon_provider_1.icon)('chevron-down', 14)}</span>
              </div>
            ...
            <div class="live-filter-section">
              <div class="live-filter-section-header" onclick="toggleFilterSection('files')">
                ...
                <span class="live-filter-chevron" id="live-filter-chevron-files">${(0, icon_provider_1.icon)('chevron-down', 14)}</span>
              </div>
            ...
            <div class="live-filter-section">
              <div class="live-filter-section-header" onclick="toggleFilterSection('suites')">
                ...
                <span class="live-filter-chevron" id="live-filter-chevron-suites">${(0, icon_provider_1.icon)('chevron-down', 14)}</span>
              </div>
        ...
        <div class="live-content">
          <div class="live-elapsed-row">
            ...
            <span class="live-elapsed-value" id="live-elapsed">${(0, utils_1.formatDuration)(totalDuration)}</span>
          </div>
        ...
        const failCardId = (0, utils_1.sanitizeId)(r.testId);
        return `<div class="live-failure-item live-failure-clickable" data-testid="${(0, utils_1.escapeHtml)(r.testId)}" onclick="selectTest('${failCardId}'); switchView('tests');">
                <div class="live-failure-item-main">
                  ...
                  <div class="live-failure-item-text">
                    <div class="live-failure-title">${(0, utils_1.escapeHtml)(r.title)}</div>
                    <div class="live-failure-file">${(0, utils_1.escapeHtml)(r.file)}${r.retry > 0 ? ` (retry ${r.retry})` : ''}</div>
                    ${r.error ? `<div class="live-failure-error">${(0, utils_1.escapeHtml)(r.error.split('\\n')[0])}</div>` : ''}
                  </div>
                </div>
  ...
  <div class="search-modal" id="search-modal" role="dialog" aria-modal="true" aria-labelledby="search-modal-title" aria-hidden="true">
    ...
    <div class="search-modal-content">
      <div class="search-modal-header">
        <span class="search-modal-icon" aria-hidden="true">${(0, icon_provider_1.icon)('search')}</span>
  ...
  <div id="test-cards-data" style="display: none;">
    ${results.map(test => (0, card_generator_1.generateTestCard)(test, showTraceSection, quarantinedTestIds)).join('\n')}
  </div>
...
${liveConfigured ? `
  ...
  (function initLiveView() {
    var liveJsonlFile = ${JSON.stringify((0, utils_1.escapeHtml)(data.options.live.outputFile ?? '.smart-live-results.jsonl'))};
...
${branding?.footer ? `    <div>${(0, utils_1.escapeHtml)(branding.footer)}</div>` : ''}
 ...
 */
function generateThemeOverrides(theme) {
    if (!theme)
        return '';
    const mappings = [
...
`;
function generateStyles(passRate, cspSafe = false, theme) {
 ...
 */
function generateScripts(testsJson, includeGallery, includeComparison, enableTraceViewer, enableHistoryDrilldown, historyRunSnapshotsJson, statsData, outputBasename) {
    ...
    function showGlobalHistoryBanner(label) {
      ...
      if (viewHeader) {
        ...
        historyBanner.innerHTML = \`
          <span class="history-banner-icon">${(0, icon_provider_1.icon)('calendar')}</span>
            ...
            <div class="detail-view-content">
              ...
              \${test.error ? \`
                <div class="detail-section">
                  <div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('alert-triangle')}</span> Error</div>
                  <div class="error-box">\${escapeHtmlUnsafe(test.error)}</div>
                </div>
              \` : ''}
              \${test.steps && test.steps.length > 0 ? \`
                <div class="detail-section">
                  <div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('clock')}</span> Steps (\${test.steps.length})</div>
              ...
              \${test.aiSuggestion ? \`
                <div class="detail-section">
                  <div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('bot')}</span> AI Suggestion</div>
    ...
    function showToast(message, type = 'info') {
      ...
      const toastIcons = { success: '${(0, icon_provider_1.icon)('check', 14)}', error: '${(0, icon_provider_1.icon)('x', 14)}', info: '${(0, icon_provider_1.icon)('info', 14)}' };
    ...
    const themeConfig = {
      system:    { icon: '${(0, icon_provider_1.icon)('monitor')}', label: 'System',    attr: null },
      light:     { icon: '${(0, icon_provider_1.icon)('sun')}', label: 'Light',     attr: 'light' },
      dark:      { icon: '${(0, icon_provider_1.icon)('moon')}', label: 'Dark',      attr: 'dark' },
      ocean:     { icon: '${(0, icon_provider_1.icon)('waves')}', label: 'Ocean',     attr: 'ocean' },
      sunset:    { icon: '${(0, icon_provider_1.icon)('sunset')}', label: 'Sunset',    attr: 'sunset' },
      dracula:   { icon: '${(0, icon_provider_1.icon)('moon')}', label: 'Dracula',   attr: 'dracula' },
      cyberpunk: { icon: '${(0, icon_provider_1.icon)('zap')}', label: 'Cyberpunk', attr: 'cyberpunk' },
      forest:    { icon: '${(0, icon_provider_1.icon)('tree-pine')}', label: 'Forest',    attr: 'forest' },
      rose:      { icon: '${(0, icon_provider_1.icon)('flower-2')}', label: 'Rose',      attr: 'rose' },
    };
    ...
    function escapeHtmlUnsafe(s) {
      return String(s)
        ...
        parts.push(
          '<div class="detail-section">' +
            '<div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('clock')}</span> Step Timings</div>' +
	      ...
	      if (snapshot.attachments && snapshot.attachments.screenshots && snapshot.attachments.screenshots.length > 0) {
	        ...
	        parts.push(
	          '<div class="detail-section">' +
            '<div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('camera')}</span> Screenshot</div>' +
      ...
      if (snapshot.attachments && snapshot.attachments.videos && snapshot.attachments.videos.length > 0) {
        parts.push(
          '<div class="detail-section">' +
            '<div class="detail-label"><span class="icon">${(0, icon_provider_1.icon)('paperclip')}</span> Attachments</div>' +
            '<div class="attachments">' +
              '<a href="file://' + escapeHtmlUnsafe(snapshot.attachments.videos[0]) + '" class="attachment-link" target="_blank">${(0, icon_provider_1.icon)('film')} Video</a>' +
            '</div>' +
          '</div>'
        );
      }
