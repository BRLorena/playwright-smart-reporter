...
function resolveReport(reportPath) {
 ...
 */
function buildFilteredCommand(baseCommand, filters) {
...
function main() {
    ...
    if (options.live) {
        ...
        sseHandler = (0, sse_handler_1.buildSseHandler)(liveFilePath);
    }
    let runChild = null;
    ...
    const server = http.createServer((req, res) => {
        ...
        if (sseHandler && req.url === '/sse') {
            ...
            res.write(':ok\n\n');
            const client = { write: (data) => res.write(data), end: () => res.end() };
        ...
        if (req.url === '/run' && req.method === 'POST' && options.runCommand) {
            ...
            req.on('end', () => {
                ...
                runChild = (0, child_process_1.spawn)(fullCommand, {
                    cwd: options.runCwd,
                    shell: true,
                    detached: true,
